...
public class SSPIKerberosAuthenticator implements Authenticator {
    ...
    public User authenticate() throws Exception {
        ...
        SspiUtil.ManagedSecBufferDesc pbClientToken = null;
        log.info("--- start");
        try {
            phClientCredential = new CredHandle();
            phClientContext = new CtxtHandle();
            IntByReference pfClientContextAttr = new IntByReference();
            pbClientToken = new SspiUtil.ManagedSecBufferDesc(Sspi.SECBUFFER_TOKEN, Sspi.MAX_TOKEN_SIZE);
            int clientRc = -1;
            log.info("- before Secur32.INSTANCE.AcquireCredentialsHandle");
            TimeStamp ptsClientExpiry = new TimeStamp();
            int result = Secur32.INSTANCE.AcquireCredentialsHandle(null, "Kerberos", Sspi.SECPKG_CRED_OUTBOUND, null, null, null,
            ...
            clientRc = Secur32.INSTANCE.InitializeSecurityContext(phClientCredential, null, LoginModuleResources.getServerPrincipal(),
                    Sspi.ISC_REQ_CONNECTION, 0, Sspi.SECURITY_NATIVE_DREP, null, 0,
            ...
            User user = WFEConnection.getAuthenticationAPI().authenticateByKerberos(pbClientToken.getBuffer(0).getBytes());
